...
class ReportController extends Controller
{
	...
	public function incomeReport(Request $req)
	{
		...
		$data = $this->model->booking()
					 ->has('itinerary_detail')
					 ->get();
		return view('report.income_report.income_report',compact('data'));
	}
	...
	public function salesReport(Request $req)
	{
		...
		return view('report.sales_report.sales_report');
	}

	public function datatableSalesReport(Request $req)
	{
	...
	public function getDataDestinationSalesReport(Request $req)
	{
		$data = $this->model->destination()
						->get();
		$dataSets = [];
	...
	{
		$data = $this->model->payment_history()
					 ->whereHas('payment_history_d',function($q) use ($req){
						$q->where('date','>=',$req->start);
						$q->where('date','<=',$req->end);
					 })
					 ->get();

		return view('report.payment_report.payment_report',compact('data'));
	}
}
